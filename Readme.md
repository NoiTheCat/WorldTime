...
```jsonc
{
    ...
    "Database": {
        ...
        "Database": "worldtime", // optional
        "Username": "worldtime", // required
        "Password": "worldtime" // required; no other authentication methods are currently supported
    }
}
```
...
To run the bot:
```sh
$ dotnet run -p src/WorldTime -c Release -- -c path/to/config.json
```
